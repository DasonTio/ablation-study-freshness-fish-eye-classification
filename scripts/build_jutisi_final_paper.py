...
def run_figure_builder() -> None:
    ...
    subprocess.run([
        "python",
        str(ROOT / "scripts" / "make_stage_xai_figure.py"),
        "--skip-if-exists",
    ], check=True)
...
def build_doc() -> None:
    ...
    add_h2(doc, "G. Stage-Wise Grad-CAM Behavior")
    add_paragraph(doc, "Figure 8 compares representative Grad-CAM maps for the three stages that matter to the paper's logic: a CLAHE-trained model, the primary second-order pooling candidate, and the final GAP Swin-Tiny classifier. The rows use matched examples from the three freshness levels, so the visual comparison follows the same evidence chain as the tables.")
    add_figure(doc, "results/figures/jutisi/figure8_stage_xai_3x3.png", "Figure 8. Stage-wise 3x3 Grad-CAM comparison for CLAHE, second-order pooling, and final GAP Swin-Tiny.", 5.8)
